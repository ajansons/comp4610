...
public abstract class Line {
	...
	LineMode linemode =  LineMode.BASIC;
			...
			void line(BufferedImage buf, int x0, int y0, int xn, int yn, int rgb) {
				...
				case BASIC: 
					...
					for (double t = 0.0; t <= 1.0; t += inc){
						double px = t*xd + x0;
						double py = t*yd + y0;
						buf.setRGB((int)Math.round(px), (int)Math.round(py), rgb);
					}
				...
				case DDA:
					...
					buf.setRGB((int)Math.round(x), (int)Math.round(y), rgb);
				...
				case BRESENHAM:
					...
					//Make sure we can deal with all directions
					if (xn >= x0){
						dx = xn - x0;
						xincr = 1;
					}else{
				...
				default: 
					System.out.println("Please select a valid LineMode");
				break;
				}
			}
		};
	...
	private static Color addColour(Color c, int toAdd){
		
		int r = Math.min(255, c.getRed()+toAdd);
		int g = Math.min(255, c.getGreen()+toAdd);
		int b = Math.min(255, c.getBlue()+toAdd);
